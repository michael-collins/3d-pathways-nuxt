<template>
    ...
    <div  v-else v-for="record in records" :key="record.id" class="border-b-2 border-base-300 pb-4 flex md:flex-row flex-col">
        ...
        <div class="block float-none md:grow">
          <NuxtLink :to="`/${destination}/${record.id}`" :key="record.id" :aria-label="`Link to ${record.fields.name}`" :aria-live="'polite'" class="hover:underline ">
        ...
        <div class="mx-auto mt-4  flex items-end">
          <NuxtLink :to="`/${destination}/${record.id}`" :key="record.id" :aria-label="`Link to ${record.fields.name}`" :aria-live="'polite'" class="btn btn-ghost ">
            View
          </NuxtLink>
        </div>
    </div>
