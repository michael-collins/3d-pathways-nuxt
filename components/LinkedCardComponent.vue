<template>
    ...
    <div v-else v-for="record in records" :key="record.id"
      class="group flex flex-col bg-base-100 border shadow-sm rounded-xl hover:shadow-md transition min-h-32 cursor-pointer sm:m-1">
      <NuxtLink :to="`/${destination}/${record.id}`" class="p-4 md:p-5">
