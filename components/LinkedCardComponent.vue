<template>
   ...
   > -->
    <NuxtLink  v-else v-for="record in records" :key="record.id" :to="`/${destination}/${record.id}`" :aria-label="`Link to ${record.fields.name}`" :aria-live="'polite'" class="group flex flex-col bg-base-100 border shadow-sm rounded-xl hover:shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-100 focus:ring-indigo-500 transition min-h-32 cursor-pointer sm:m-1 p-4 md:p-5">
