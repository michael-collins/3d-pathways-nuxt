<template>
  <article ref="articleElement">
    <div v-if="record" class="container max-w-3xl px-4 pt-6 lg:pt-10 pb-12 sm:px-6 lg:px-8 mx-auto">
      ...
      <!-- iframe -->
      <IframeConfigGenerator v-if="showIframe" :articleHeight="articleHeight" :iframeUrl="iframeUrl" :record="record" >
        <!-- Menu of Checkboxes -->
      ...
      </IframeConfigGenerator>
      <LegacyLicenseComponent v-if="showLicense" :work="record.fields.name" :currentUrl="route.path" :license="license" :author="record.fields.author" :authorURL="record.fields.authorURL" title="License" />
    </div>
  </article>
</template>
